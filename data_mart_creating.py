...
def create_data_marts(date):
    ...
    query_job = client.query(cpi_query)
    results = query_job.result()  # Ждем завершения выполнения запроса

    print(f'<{log_datetime}>: Data Mart создан')

    print(f'<{log_datetime}>: Начинается процесс создания Data Mart для получения REVENUE')
    ...
    query_job = client.query(revenue_query)
    results = query_job.result()  # Ждем завершения выполнения запроса

    print(f'<{log_datetime}>: Data Mart создан')

    print(f'<{log_datetime}>: Начинается процесс создания Data Mart для получения ROAS')
